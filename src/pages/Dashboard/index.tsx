...
const Dashboard = () => {
  ...
  const ftstatusMapper = (
    ...
    if (status === 'STBY') {
      return (
        <span className="px-3 py-1 bg-slate-200 rounded-xl">{status}</span>
      );
    }

    if (status === 'RUNNING' && bd_end !== null) {
      return <span className="px-3 py-1 bg-green-200 rounded-xl">RFU</span>;
    }

    if (activity == 'SERVICE') {
      return (
        <span className="px-3 py-1 bg-yellow-200 rounded-xl">{activity}</span>
      );
    }

    return <span className="px-3 py-1 bg-red-200 rounded-xl">BD</span>;
  };

  const stockStatusMapper = (qty: number) =>{
      if(qty>80){
        return <span className='px-3 py-1 bg-green-200 rounded-xl'>{qty}%</span>
      }
      if(qty>60){
        return <span className='px-3 py-1 bg-yellow-200 rounded-xl'>{qty}%</span>
      }
      return <span className='px-3 py-1 bg-red-200 rounded-xl'>{qty}%</span>
  }
        ...
        <div className="flex flex-wrap items-center">
          <div className="w-full border-stroke dark:border-strokedark xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-5">
              ...
              <div className="content__container grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="manpower__setting col-span-1  overflow-y-hidden h-125">
                ...
                <div className="stock__status col-span-1  ">
                  ...
                  <div className="overflow-x-auto  overflow-y-scroll h-125">
                    <table className="min-w-full divide-y divide-gray-200 rounded-lg shadow-md">
                      ...
                      <tbody className="divide-y divide-slate-300 bg-slate-50 dark:bg-slate-800 dark:divide-slate-700 text-black dark:text-white">
                        {stStatusData.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                              {formatNumberWithSeparator(parseFloat((item.qty_liter | 0).toFixed(0)))}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                              {stockStatusMapper(Math.floor(((item.qty_liter / item.max_capacity)*100)))}
                            </td>
                          </tr>
                        ))}
                      </tbody>
