...
const DailyStockTakingOil: React.FC = () => {
  ...
  if (!key || !selectedStorage || !selectedMaterial || tankNumber == null) return;

  const handler = setTimeout(async () => {
    try {
      // compute qty otomatis
      ...
      console.warn('Auto compute qty gagal', e);
      // toast/alert kalau mau
    }
  }, 600);

  return () => clearTimeout(handler);
}, [inputValue, selectedUOI, selectedStorage, selectedMaterial, tankNumber]);
  ...
  try {
    ...
    if (computed == null) throw new Error('Qty not found');
    setQty(computed);
    // simpan ke localStorage seperti sebelumnya …
  } catch (err) {
    console.error(err);
    // pakai toast kalau ada library; sementara pakai alert:
    alert('Gagal menghitung qty untuk input ini');
  }
};
