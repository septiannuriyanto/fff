...
export const DroppableConsumer: React.FC<{
  ...
  return (
    <div
      ...
      className={`relative p-3 pb-5 border rounded-lg text-center transition-all duration-200 w-28 min-h-[140px] flex flex-col overflow-visible ${
        getBackgroundColor()
      } ${
        getBorderColor()
      } ${
    ...
    >
      {/* RIGHT TOP RIBBON - FLAT (UNIT ID) */}
      <div className="absolute top-0 right-0 bg-gradient-to-l from-slate-600 to-slate-700 text-white text-[9px] font-bold px-2 py-0 rounded-bl-lg shadow-md z-20 pointer-events-none">
      ...
      {hasFilledTank && (
        <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between z-10 pointer-events-none">
          <div className={`${ribbonColors.bg} ${ribbonColors.text} text-[9px] font-bold px-2 py-1 shadow-sm flex-1 text-left`}>
            {consumer.current_tank_nomor_gt || 'N/A'}
          </div>
          <div className={`${ribbonColors.bg} ${ribbonColors.text} text-[9px] font-bold px-2 py-1 shadow-sm flex-1 text-right`}>
            {qtyText}
          </div>
        </div>
      )}
