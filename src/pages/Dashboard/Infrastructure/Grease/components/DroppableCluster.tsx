...
export const DroppableCluster: React.FC<{
  ...
  onConsumerTap?: (consumer: ConsumerUnit) => void;
  ...
  const getDroppableStyles = useCallback(() => {
    if (isRegister) return 'border-purple-300 bg-purple-50';
    if (isOver && !isConsumerDroppoint) return 'border-green-400 bg-green-50 shadow-lg';
    if (isOver && isConsumerDroppoint) return 'border-indigo-400 bg-indigo-50 shadow-lg';
  ...
  return (
    ...
    >
      ...
      {/* Tanks Section — hanya tampil kalau benar-benar ada tank */}
      {tanks.length > 0 && (
        <div className="border-t border-gray-200 pt-3">
          <div className="flex gap-3 flex-wrap bg-gray-50 rounded-lg p-3">
            {tanks
      ...
      {isConsumerDroppoint && (
        <div className="border-t border-gray-200 pt-3 mt-3">
          ...
          <h5 className="text-sm text-gray-700 mb-2">
            Drag tank dari Main Warehouse ke salah satu Unit Konsumen di bawah ini,
            atau klik unit untuk melakukan penggantian/install GT baru.
          </h5>
