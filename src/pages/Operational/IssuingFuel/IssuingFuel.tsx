...
const IssuingFuel: React.FC = () => {
  ...
  // Auto-scroll to latest (rightmost) data on load
  useEffect(() => {
  ...
  useEffect(() => {
    if (unitScrollRef.current) {
      unitScrollRef.current.scrollLeft = unitScrollRef.current.scrollWidth;
    }
  }, [unitData]);
  ...
  const getBaseOptions = (): ApexOptions => ({
    ...
    // Disable built-in responsive scale-down so the scroll container controls sizing
    responsive: [],
    theme: {
  ...
  return (
    <ThemedPanelContainer title="Issuing Analysis">
      <div className="flex flex-col space-y-6 animate-fade-in-up pb-10">
        ...
        <ThemedGlassmorphismPanel activeTheme={activeTheme} className="p-5 overflow-visible">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex p-1 bg-element-light dark:bg-element-dark rounded-lg">
              <button
                ...
                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${trendView === 'Total'
              ...
              <button
                ...
                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${trendView === 'Phase'
                    ? 'bg-blue-600 text-white shadow-md'
                    : 'text-text-secondary-light dark:text-text-secondary-dark hover:text-text-primary-light dark:hover:text-text-primary-dark'
                  }`}
              >
          ...
          <div className="w-full overflow-x-auto" ref={trendScrollRef}>
            <div style={{ width: '100%', minWidth: `${Math.max(trendData.length * 56, 600)}px`, height: '350px' }}>
              {loading ? (
        ...
        <ThemedGlassmorphismPanel activeTheme={activeTheme} className="p-5 overflow-visible">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            <h2 className="text-lg font-semibold flex items-center gap-2 text-text-primary-light dark:text-text-primary-dark">
              ...
              Average Usage By Storage, Categorized by Phase
            </h2>
          ...
          <div className="w-full overflow-x-auto" ref={unitScrollRef}>
            <div style={{ width: '100%', minWidth: `${Math.max(unitData.length * 72, 700)}px`, height: '430px' }}>
              {loading ? (
                <div className="w-full h-full flex items-center justify-center text-text-secondary-light dark:text-text-secondary-dark">Loading chart data...</div>
              ) : (
                <ReactApexChart
