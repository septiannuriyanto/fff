...
interface FuelConsPercentageChartProps {
  ...
  subtitle?: string;
  period: string;
  onClickChart?: (values: { x_value: string }) => void;
  onClickPeriode?: (values: { periode: string }) => void;
}
...
const FuelConsPercentageChart: React.FC<FuelConsPercentageChartProps> = ({
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: data!.map((item) => item.desc),
    },
    // yaxis: {
    //   labels: {
  ...
  async function parseChartData(sourceData: any[]) {
    ...
    sourceData.map((item) => {
      var newseri = {
        ...
        goals: [
          {
            name: 'Threshold',
            value: (1).toPrecision(2),
            strokeWidth: 100 / data!.length,
            strokeHeight: 4,
            strokeColor: '#3C50E0',
          },
        ],
      };
      chartContent.push(newseri);
    });
    setchartContentData(chartContent);
    return chartContent;
  }

  async function mapData() {
    const dataMap = await parseChartData(data!);
    setState({
      series: [
        {
          name: 'Actual',
        ...
        <div>
          ...
          {showPeriodPicker? <select
            ...
            >
              ...
              <option value="This Month" className="dark:bg-boxdark">
                This Month
              </option>
              <option value="Last Month" className="dark:bg-boxdark">
                Last Month
              </option>
...
export default FuelConsPercentageChart;
